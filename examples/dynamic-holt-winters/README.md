...
```bash
kubectl get configmap hpa-plus-dynamic-holt-winters-data -o=json | jq -r '.data.data | fromjson | .modelHistories["HoltWintersPrediction"].replicaHistory[] | .time,.replicas'
```
...
- `models` - predictive models to apply.
  ...
  - `holtWinters` - Holt-Winters specific configuration.
    * `runtimeTuningFetchHook` - This is a [hook](https://github.com/cslab-ntua/HPA-Plus/tree/master/docs/user-guide/hooks.md)
