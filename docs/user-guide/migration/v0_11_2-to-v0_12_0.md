...
There are some minor breaking changes moving from HPA+ `v0.11.2` to `v0.12.0` - this
page will detail how you can migrate between versions and handle these breaking changes.

This is split into two parts, for most users of the HPA+ the only part that will apply is [Migrating Predictive
Horizontal Pod Autoscaler definitions](#migrating-hpa-plus-definitions), the section
...
## Migrating HPA+ definitions

The HPA+ no longer has the `downscaleStabilization` field, this has been replaced by
