...
Some key differences for this is that you no longer need the custom-pod-autoscaler-operator to run HPA+s, and if
you have no other Custom Pod Autoscalers on your cluster you can safely uninstall this operator.

Now to deploy HPA+s you need to install the HPA+ operator instead, you see how to
...
In `v0.10.0` the `simple-linear` YAML to deploy the HPA+ was this:
...
spec:
  template:
    spec:
      containers:
      - name: simple-linear
        image: dimitrisde/hpa-plus:v0.10.0
...
We no longer need to provide containers/an image for the HPA+ to use, the operator handles all of the processing
...
full](https://github.com/cslab-ntua/HPA-Plus/blob/master/CHANGELOG.md).
